...
export function listActiveReminders(
  ...
  return reminderDao.listOpenForDate(db, date)
}

export function dismissReminder(db: SqliteDatabase, idRaw: string): TeacherReminderRow {
  const id = idRaw.trim()
  if (!id) throw new HttpError(400, 'id is required')
  const row = reminderDao.findById(db, id)
  if (!row) throw new HttpError(404, 'reminder not found')
  if (row.status !== 'open') {
    return row
  }
  const resolvedAt = Date.now()
  reminderDao.updateStatus(db, id, 'dismissed', resolvedAt)
