...
function resolveMissedFromDate(
  ...
  const sy = schoolYearDao.getActiveSchoolYear(db, userId)
...
export function listDueItems(
  ...
  syncAttendanceDueReminder(db, userId, date, 'AM')

  syncAttendanceDueReminder(db, userId, date, 'PM')

  return listActiveReminders(db, userId, date).map(reminderToDueItem)
...
export function listMissedAttendanceDueItems(
  ...
  const from =
    ...
    typeof fromRaw === 'string' && fromRaw.trim()
      ...
      : resolveMissedFromDate(db, userId, today)
  ...
  const saved = new Set(
    ...
    attendanceDao
      ...
      .listSessionsInRange(db, userId, from, to)
...
export function listAllDueItems(
  ...
  const todayItems = listDueItems(db, userId, dateRaw, now)

  const missed = listMissedAttendanceDueItems(db, userId, undefined, undefined, now)

  const quarter = listQuarterDeadlineDueItems(db, userId, now)
