export const TEACHER_REMINDER_QUERIES = {
  findOpenByTypeDatePeriod: `
    SELECT id, user_id, type, date, period, status, message, created_at, resolved_at
  ...
  resolveOpenByDatePeriod: `
    ...
    WHERE user_id = @user_id AND type = @type AND date = @date AND period = @period AND status = 'open'
  `,
  findById: `
    SELECT id, user_id, type, date, period, status, message, created_at, resolved_at
    FROM teacher_reminders
