...
### CORS origins to allowlist

**Web:** your static host origin(s), e.g. `https://app.example.com`

**Capacitor (browser-based requests only):** native HTTP clients do not send browser CORS preflights, but WebView fetches may. Verify on device; common values:

- `capacitor://localhost`
- `https://localhost`
- `ionic://localhost` (if applicable)
...
- GAP-005 mobile secure token storage
